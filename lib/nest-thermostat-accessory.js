...
class NestThermostatAccessory extends NestDeviceAccessory {
    constructor(conn, log, device, structure, platform) {
       ...
       */
        let tempStep = 0.001, minTemp, maxTemp;
        if (!this.usesFahrenheit()) {
            minTemp = 10;
            maxTemp = 32;
        } else {
            minTemp = this.fahrenheitToCelsius(50);
            maxTemp = this.fahrenheitToCelsius(90);
        }

        thermostatService.getCharacteristic(Characteristic.CurrentTemperature).setProps({ minStep: tempStep });
        thermostatService.getCharacteristic(Characteristic.TargetTemperature).setProps({ minStep: tempStep, minValue: minTemp, maxValue: maxTemp });
        thermostatService.getCharacteristic(Characteristic.CoolingThresholdTemperature).setProps({ minStep: tempStep, minValue: minTemp, maxValue: maxTemp });
        thermostatService.getCharacteristic(Characteristic.HeatingThresholdTemperature).setProps({ minStep: tempStep, minValue: minTemp, maxValue: maxTemp });
    ...
    getCurrentTemperature() {
        return this.device.current_temperature || this.device.backplate_temperature;
    }

    getBackplateTemperature() {
        return this.device.backplate_temperature || this.device.current_temperature;
    }
    ...
    getCoolingThresholdTemperature() {
        ...
        case 'eco':
            ...
            return this.device.eco_temperature_high || this.device.away_temperature_high;
        default:
            return this.device.target_temperature_high || this.device.target_temperature;
        }
    }
    ...
    getHeatingThresholdTemperature() {
        ...
        case 'eco':
            // away_temperature deprecated in v5. in v6 use eco_temperature but if undefined, fallback to away_temperature
            return this.device.eco_temperature_low || this.device.away_temperature_low;
        default:
            return this.device.target_temperature_low || this.device.target_temperature;
        }
    }
    ...
    formatAsDisplayTemperature(t) {
        return t + ' °C / ' + (0.5 * Math.round(this.celsiusToFahrenheit(t) * 2)) + ' °F';
    }
