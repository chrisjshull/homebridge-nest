...
NestThermostatAccessory.prototype.getTargetTemperature = function () {
    this.verbose('getTargetTemperature', this.device.hvac_mode, this.device.hvac_state);
    switch (this.device.hvac_mode) {
    case 'eco':
...
NestThermostatAccessory.prototype.setTargetHeatingCooling = function (targetHeatingCooling, callback) {
    ...
    return this.setPropertyAsync('shared', 'hvac_mode', val, 'target heating cooling').asCallback(callback).finally(() => this.conn.mutex.endPropertyUpdate(this.conn.mutex.updateStates.targetHeatingCooling));
};
...
NestThermostatAccessory.prototype.setTargetTemperature = function(targetTemperature, callback) {
    this.verbose('setTargetTemperature', targetTemperature);
    ...
    } else if (this.device.hvac_mode === 'range') {
        return void callback();
    }
...
NestThermostatAccessory.prototype.setCoolingThresholdTemperature = function(targetTemperature, callback) {
    ...
    if (this.device.hvac_mode == 'eco') {
        setting = 'away_temperature_high';
    } else if (this.device.hvac_mode == 'cool') {
        // Work around for iOS bug - should not be adjusting heating threshold unless in Auto mode
...
NestThermostatAccessory.prototype.setHeatingThresholdTemperature = function(targetTemperature, callback) {
    ...
    if (this.device.hvac_mode == 'eco') {
        setting = 'away_temperature_low';
    } else if (this.device.hvac_mode == 'heat') {
        // Work around for iOS bug - should not be adjusting heating threshold unless in Auto mode
        setting = 'target_temperature';
    }
    this.device[setting] = targetTemperature;
...
NestThermostatAccessory.prototype.setTargetTemperatureDebounced = debounce(function (targetTemperature, callback) {
    ...
    if (mode === 'off') {
        ...
        if ((ambient < targetTemperature) || (!this.device.can_cool)) {
            ...
            promise = promise.then(() => this.setPropertyAsync('shared', 'hvac_mode', 'heat', 'target heating cooling')).delay(5000);
        } else if ((ambient > targetTemperature) || (!this.device.can_heat)) {
            this.device.hvac_mode = 'cool';
            promise = promise.then(() => this.setPropertyAsync('shared', 'hvac_mode', 'cool', 'target heating cooling')).delay(5000);
        } else {
            return void callback();
        }
    } else if (mode === 'eco') {
...
NestThermostatAccessory.prototype.setFanState = function (targetFanState, callback) {
    ...
    return this.setPropertyAsync('device', 'fan_timer_active', Boolean(targetFanState), 'fan enable/disable')
        .asCallback(function () {
...
NestThermostatAccessory.prototype.setEcoMode = function (eco, callback) {
    ...
    return this.setPropertyAsync('shared', 'hvac_mode', eco ? 'eco' : 'eco-off', 'target heating cooling').asCallback(callback).finally(() => this.conn.mutex.endPropertyUpdate(this.conn.mutex.updateStates.ecoMode));
};
