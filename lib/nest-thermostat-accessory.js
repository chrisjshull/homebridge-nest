...
NestThermostatAccessory.prototype.setCoolingThresholdTemperatureDebounced = debounce(function (targetTemperature, callback) {
  return Promise.resolve()
    .then(() => this.updateTargetWithCorrectUnitsAsync('target_temperature_', targetTemperature, "cooling threshold temperature"))
    .asCallback(callback);
}, 5000);

NestThermostatAccessory.prototype.setHeatingThresholdTemperatureDebounced = debounce(function (targetTemperature, callback) {
  return Promise.resolve()
    .then(() => this.updateTargetWithCorrectUnitsAsync('target_temperature_', targetTemperature, "heating threshold temperature"))
    .asCallback(callback);
}, 5000);
