...
function NestThermostatAccessory(conn, log, device, structure, platform) {
   ...
   */
    ...
    if (this.platform.shouldEnableFeature('Thermostat.TemperatureSensors')) {
        Object.keys(this.device.tempSensors).forEach(sensorId => {
            ...
            this.bindCharacteristic(tempService, Characteristic.CurrentTemperature, 'Temperature', () => this.getSensorTemperature(sensor.sensor_id), null);
        });
    }
...
NestThermostatAccessory.prototype.setEcoMode = function (eco, callback) {
    ...
    return this.updateDevicePropertyAsync('hvac_mode', eco ? 'eco' : 'eco-off', 'target heating cooling').asCallback(callback).finally(() => this.conn.mutex.endTemperatureUpdate(this.conn.mutex.updateStates.ecoMode));
};
