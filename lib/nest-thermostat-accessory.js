...
class NestThermostatAccessory extends NestDeviceAccessory {
    constructor(conn, log, device, structure, platform) {
       ...
       */
        let minSetTemp, maxSetTemp, minGetTemp, maxGetTemp, minSetHumidity, maxSetHumidity;
        this.tempStep = 0.1;
        minSetHumidity = 0;
        maxSetHumidity = 100;
        this.humidityStep = 1;
        ...
        thermostatService.getCharacteristic(Characteristic.TargetRelativeHumidity).setProps({ minStep: this.humidityStep, minValue: minSetHumidity, maxValue: maxSetHumidity });
        ...
        bindCharacteristic(Characteristic.TargetRelativeHumidity, 'Target humidity', this.getTargetHumidity, this.setTargetHumidity, function(val) {
            return val + '%';
        });;
    ...
    getTargetHumidity() {
        this.conn.verbose('getTargetHumidity', this.device.hvac_mode, this.device.hvac_state, this.device.target_humidity);
        return this.device.target_humidity;
    }
    ...
    setTargetHumidity(targetHumidity, callback) {
        this.conn.verbose('setTargetHumdity', this.device.hvac_mode, targetHumidity);

        let deviceType = 'device';
        let setting = 'target_humidity';
        let promise = Promise.resolve();

        this.log('Setting humidity to ' + targetHumidity);
        return promise.then(() => this.setPropertyAsync(deviceType, setting, targetHumidity, 'target humidity')).asCallback(callback);
    }
