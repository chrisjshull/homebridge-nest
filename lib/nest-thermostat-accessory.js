...
NestThermostatAccessory.prototype.getTargetTemperature = function () {
	switch (this.device.hvac_mode) {
		...
		case "eco":
			var high, low;
			low = this.device.eco_temperature_low_c;
			high = this.device.eco_temperature_high_c;
			return ((low + high) / 2);
		default:
	...
	switch (this.device.hvac_mode) {
		...
		default:
			return this.cancelAutoAway()
				.then(this.updateDevicePropertyAsync.bind(this, "target_temperature_c", targetTemperature, "target temperature"))
				.asCallback(callback);
	}
}, 5000);
...
NestThermostatAccessory.prototype.setCoolingThresholdTemperatureDebounced = debounce(function (targetTemperature, callback) {
	switch (this.device.hvac_mode) {
		case "heat-cool":
			return this.cancelAutoAway()
				.then(this.updateDevicePropertyAsync.bind(this, "target_temperature_high_c", targetTemperature, "cooling threshold temperature"))
				.asCallback(callback);
		default:
			return null;
	}
...
NestThermostatAccessory.prototype.setHeatingThresholdTemperatureDebounced = debounce(function (targetTemperature, callback) {
	switch (this.device.hvac_mode) {
		case "heat-cool":
			return this.cancelAutoAway()
				.then(this.updateDevicePropertyAsync.bind(this, "target_temperature_low_c", targetTemperature, "heating threshold temperature"))
				.asCallback(callback);
		default:
			return null;
	}
