...
function NestProtectAccessory(conn, log, device, structure) {
    ...
    var smokeSvc = this.addService(Service.SmokeSensor)
        .setCharacteristic(Characteristic.Name, this.device.name + " " + "Smoke Sensor");
    ...
    var coSvc = this.addService(Service.CarbonMonoxideSensor)
        .setCharacteristic(Characteristic.Name, this.device.name + " " + "CO Sensor");
...
var formatOnlineStatus = function (val) {
    switch (val) {
        case true:
            return 'online';
        case false:
