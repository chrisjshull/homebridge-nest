...
function NestProtectAccessory(conn, log, device, structure) {
    ...
    var batSvc = this.addService(Service.BatteryService);
    this.bindCharacteristic(batSvc, Characteristic.StatusLowBattery, "Battery status",
        getBatteryHealth.bind(this), null, formatStatusLowBattery.bind(this));
    this.bindCharacteristic(batSvc, Characteristic.BatteryLevel, "Faked battery level", getFakedBatteryLevel.bind(this));
...
var getFakedBatteryLevel = function () {
    switch (getBatteryHealth()) {
        case Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW:
            return 20;
        default:
            return 100;
