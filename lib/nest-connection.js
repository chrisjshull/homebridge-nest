...
Connection.prototype.updateData = function() {
    ...
    return rp({
        ...
        if (this.mutex.isApiUpdatePending() || this.mutex.isTemperatureUpdatePending()) {
            this.verbose('API: get status update deferred [after call, before data update] as property change active');
            return null;
        } else {
            return data;
        }
    ...
    if (deviceType == 'structure') {
        ...
        if (property == 'hvac_mode') {
            if (['eco', 'eco-off'].includes(value)) {
                ...
                body = { target_temperature_type: value == 'heat-cool' ? 'range' : value };
            }
        }
