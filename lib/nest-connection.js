            ...
            if (this.config.googleAuth) {
                ...
                try {
                    ...
                    if (result.status == 'OK' && result.access_token) {
                        ...
                        console.log(result);
                        throw({ code: result.code || 900 });
                    }
                } catch(error) {
                    this.error('Access token acquisition failed:', error.statusCode || (error.cause && error.cause.code) || error.code);
                }
            }
