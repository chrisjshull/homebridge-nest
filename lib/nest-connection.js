...
Connection.prototype.updateData = function() {
    ...
    .then(body => {
        ...
        Object.keys(structures).forEach(structureId => {
            ...
            swarm.map(unit => unit.split('.')).forEach(unit => {
                ...
                if (deviceType == 'device') {
                    ...
                    thisDevice.name = thisDevice.name || whereLookup[thisDevice.where_id] || 'Nest Thermostat';
                    ...
                    // Add data for any Nest Detects
                    thisDevice.detectSensors = {};
                    if (rcs_settings[deviceId] && rcs_settings[deviceId].associated_rcs_sensors) {
                        rcs_settings[deviceId].associated_rcs_sensors.forEach(sensorName => {
                            ...
                            if (thisSensor) {
                                thisDevice.detectSensors[sensorId] = {
                                    sensor_id: sensorId,
                                    name: whereLookup[thisSensor.where_id] || 'Nest Detect',
                    ...
                    thisDevice.device_id = deviceId;
                    thisDevice.name = thisDevice.description || whereLookup[thisDevice.where_id] || 'Nest Protect';
        ...
        /* }).catch(error => {
        this.log('Nest API call to update status returned an error: ' + (error.statusCode || error));
        throw(error); */
    });
};
