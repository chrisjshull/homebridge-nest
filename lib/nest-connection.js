    ...
    return new Promise(resolve => {
        ...
        rp(req).then(body => {
            ...
            if (error.statusCode == 401 && error.response && error.response.body && error.response.body.truncated_phone_number) {
                ...
                Promise.delay(API_AUTH_FAIL_RETRY_DELAY_SECONDS * 1000).then(() => this.auth(this.config.email, this.config.password, this.config.pin, this.config.access_token)).then(connected => resolve(connected));
            }
        });
    });
};
    ...
    this.updateData().then(data => {
        ...
        if (!error.cause || (error.cause && error.cause.code != 'ESOCKETTIMEDOUT')) {
            ...
            if (error.statusCode == 401 || error.statusCode == 403 || (error.cause && error.cause.code == 'ECONNREFUSED')) {
                ...
                this.log('Reauthenticating on Nest service in ' + API_AUTH_FAIL_RETRY_DELAY_SECONDS + ' seconds.');
                return Promise.delay(API_AUTH_FAIL_RETRY_DELAY_SECONDS * 1000).then(() => this.auth(this.config.email, this.config.password));
            } else {
...
Connection.prototype.pushUpdates = function(data) {
    ...
    return Promise.delay(additionalDelay).then(() => rp({
        ...
        if (error.statusCode == 401 || error.statusCode == 403 || (error.cause && ['ECONNREFUSED','ETIMEDOUT','ENOTFOUND'].includes(error.cause.code))) {
            ...
            return this.auth(this.config.email, this.config.password).then(() => this.pushUpdates(data));
        }
    });
};
