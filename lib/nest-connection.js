...
function UpdateQueueItem(nestconn, path, data, interval) {
	this.execInfo = {
		nestconn: nestconn,
		path: path,
		data: data
	};
	this.id = setTimeout(execUpdateQueueItem.bind(this), interval);
}

UpdateQueueItem.prototype.clear = function() {
	if (this.cleared || !this.execInfo) return false;
	this.execInfo = null;
...
var execUpdateQueueItem = function() {
	// Ensure it is OK to execute
	var execInfo = this.execInfo;
	this.execInfo = null;
	if (!execInfo) return;


	if (execInfo.nestconn._updateQueue[execInfo.path] != this) return;
	var child = execInfo.nestconn.conn.child(execInfo.path);
	Promise.fromCallback(child.set.bind(child, execInfo.data))
		.tap(function() {
			execInfo.nestconn.debug("Finished " + execInfo.path + " to " + execInfo.data);
		})
		.catch(function(err) {
			execInfo.nestconn.error(err);
		});
};

Connection.prototype.update = function(path, data) {
	this.debug("Starting " + path + " to " + data);
	var oldTimer = this._updateQueue[path];
	if (oldTimer) oldTimer.clear();
	this._updateQueue[path] = new UpdateQueueItem(this, path, data, 5000);
