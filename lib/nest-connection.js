...
// Interval between camera status poll requests
const API_CAMERA_POLL_DELAY_SECONDS = 4;
...
function Connection(config, log, verbose, fieldTestMode) {
    ...
    /* let protoGroups = ['nest/trait/hvac', 'nest/trait/sensor', 'nest/trait/structure', 'weave/trait/peerdevices', 'nest/trait/located', 'weave/trait/description', 'nest/trait/user', 'weave/trait/heartbeat', 'weave/common', 'weave/trait/security', 'weave/trait/power', 'nest/trait/security', 'nest/trait/detector', 'nest/messages', 'nestlabs/gateway/v1'];
    protoGroups.forEach(group => {
        protobuf.load(__dirname + '/protobuf/' + group + '.proto').then(data => this.proto[group] = data);
    }); */
...
Connection.prototype.auth = async function(preemptive) {
    let req, body;
...
Connection.prototype.updateProtobufData = function(resolve, handler) {
    var notify;
    ...
    return new Promise((res, rej) => {
        ...
        req.on('data', data => {
            ...
            if (protoBuffer.length >= pendingLength) {
                ...
                if (pendingLength == 2) {
                    this.verbose(protoMessage.toString('base64'));
                }
                ...
                if (notify || observeMessage.hasDeviceInfo) {
                    if (Object.keys(this.protobufBody).length > 0) {
...
Connection.prototype.protobufToNestLegacy = function(protobuf) {
    ...
    function translateProperty(object, propName, constructor, enumerator) {
        ...
        if (propObjects) {
            ...
            propObjects.forEach(propObject => enumerator(propObject, toLegacy(propObject.object.id)));
        }
    }
    ...
    try {
        ...
        if (object && object.message && object.message.length > 0) {
            ...
            if (object) {
                ...
                translateProperty(object, 'user_info', null, (userInfo, id) => {
                ...
                translateProperty(object, 'peer_devices', null, (peerDevice, id) => {
                    ...
                    if (!peerDevice.object.id.startsWith('STRUCTURE_')) {
                        ...
                        this.verbose('Protobuf API: structure ' + structureId + ', mounted', this.mountedDeviceCount.protobuf[structureId], 'device(s)');
                        if (oldMountedDeviceCount.protobuf[structureId] !== undefined && oldMountedDeviceCount.protobuf[structureId] !== this.mountedDeviceCount.protobuf[structureId]) {
                            this.verbose('Protobuf API: mounted device count for structure', structureId, 'has changed (protobuf):', oldMountedDeviceCount.protobuf[structureId], '->', this.mountedDeviceCount.protobuf[structureId]);
...
Connection.prototype.apiResponseToObjectTree = function(body) {
    let data = {};
    data.devices = {};
    ...
    Object.keys(structures).forEach(structureId => {
        ...
        data.devices['home_away_sensors'][structureId].serial_number = 'None';
...
Connection.prototype.dataTimerLoop = function(resolve, handler) {
    ...
    this.updateData().then(data => {
        ...
        if (!['ESOCKETTIMEDOUT','ECONNABORTED'].includes(error.code)) {
            ...
            if (error.status == 401 || error.status == 403 || ['ECONNREFUSED','ENOTFOUND','ENETUNREACH'].includes(error.code)) {
                // Token has probably expired, or transport endpoint has changed - re-authenticate
                this.log('Reauthenticating on Nest service ...');
                return this.auth().catch((error) => {
...
Connection.prototype.protobufDataTimerLoop = function(resolve, handler) {
    var apiLoopTimer;
    ...
    this.updateProtobufData(resolve, handler).then(() => {
        ...
        if (this.lastProtobufCode && this.lastProtobufCode.code == 13) {
            this.log('API observe: internal error, waiting for ' + API_RETRY_DELAY_SECONDS + ' seconds, code', this.lastProtobufCode);
            return Promise.delay(API_RETRY_DELAY_SECONDS * 1000);
            ...
            this.log('API observe: resolving null, code', this.lastProtobufCode);
            return Promise.resolve(null);
        }
    }).catch(error => {
        this.log('API observe: error', error);
        this.log('Retrying in ' + API_RETRY_DELAY_SECONDS + ' seconds.');
        return Promise.delay(API_RETRY_DELAY_SECONDS * 1000);
    }).finally(() => {
        this.log('API observe: setting issue timer.');
        apiLoopTimer = setInterval(() => {
            if (apiLoopTimer) {
                clearInterval(apiLoopTimer);
            }
...
Connection.prototype.observe = function(handler) {
    return new Promise(resolve => {
        this.protobufDataTimerLoop(resolve, handler);
    });
};

Connection.prototype.update = function(device, property, value, hvac_mode, using_protobuf) {
...
Connection.prototype.commitUpdate = function(nodeId, body, hvac_mode, using_protobuf) {
...
Connection.prototype.mergeNestWithProtobufData = function(nestApiData, data) {
...
Connection.prototype.protobufSendCommand = function(cmd, device_id, successResponse) {
...
Connection.prototype.pushUpdates = function(data) {
    ...
    return Promise.delay(additionalDelay).then(() => {
        ...
        if (error.status == 401 || error.status == 403 || ['ECONNREFUSED','ESOCKETTIMEDOUT','ECONNABORTED','ENOTFOUND','ENETUNREACH'].includes(error.code)) {
...
Connection.prototype.legacyToProtobufUpdate = function(body) {
...
Connection.prototype.filterProtobufUpdates = function(data) {
...
Connection.prototype.createNestBody = function(currentState, objects, objectList) {
    objects.forEach(obj => {
        ...
        if (key == 'structure' && cloneObj.value && cloneObj.value.swarm) {
            ...
            if (oldMountedDeviceCount.rest[structureId] !== undefined && oldMountedDeviceCount.rest[structureId] !== this.mountedDeviceCount.rest[structureId]) {
                this.verbose('REST API: mounted device count for structure', structureId, 'has changed (protobuf):', oldMountedDeviceCount.rest[structureId], '->', this.mountedDeviceCount.rest[structureId]);
                if (this.config.exitOnDeviceListChanged) {
                    process.exit(1);
                }
            }
...
function lookupTrait(proto, type_url) {
    let pbufTrait = null;
    Object.keys(proto).forEach(traitKey => {
        try {
