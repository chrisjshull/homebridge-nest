...
Connection.prototype.updateData = function() {
    ...
    let body = this.objectList.objects.length ? this.objectList : {'known_bucket_types':['device','kryptonite','shared','structure','topaz','user','where'],'known_bucket_versions':[]};
...
Connection.prototype.pushUpdates = debounce(function() {
    this.verbose('Pushing updates', Object.keys(this.pendingUpdates).length);
    ...
    return Promise.resolve(rp({
        ...
        if (error.statusCode == 401 || error.statusCode == 403 || (error.cause && error.cause.code == 'ECONNREFUSED')) {
            ...
            this.log('Reauthenticating on Nest service ...');
            this.auth(this.loggedin_email, this.loggedin_password).then(() => this.pushUpdates());
        }
    });
}, API_PUSH_DEBOUNCE_SECONDS * 1000);
