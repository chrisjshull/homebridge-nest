...
class NestLockAccessory extends NestDeviceAccessory {
    constructor(conn, log, device, structure, platform) {
        ...
        const privacyService = this.addService(Service.Switch, this.homeKitSanitize(this.device.name), 'privacy_mode.' + this.device.device_id)
        privacyService.bindCharacteristic(privacyService, Characteristic.On, 'Privacy (Target)', this.getPrivacyTarget, this.setPrivacyMode)
    ...
    getPrivacyTarget() {
        return Characteristic.Switch.On
    }    
