...
class NestLockAccessory extends NestDeviceAccessory {
    constructor(conn, log, device, structure, platform) {
        ...
        const privacyService = this.addService(Service.Switch, 'Privacy Mode', 'privacy_mode.' + this.device.device_id);
        this.bindCharacteristic(privacyService, Characteristic.On, 'Privacy Mode', this.getPrivacyModeOn, this.setPrivacyMode);
    ...
    getPrivacyModeOn() {
        return this.device.privacy_mode_on;
    }    
    ...
    setLocked(val, callback) {
        ...
        this.conn.protobufSendCommand([ cmd ], 'DEVICE_' + this.device.device_id).asCallback(callback);
    }

    setPrivacyMode(val, callback) {
        let cmd = {
            traitLabel: 'linus_lock_privacy_mode',
            command: {
                type_url: 'type.nestlabs.com/yale.trait.LinusLockPrivacyModeTrait.PrivacyModeStateChangeRequest',
                value: {
                    privacyModeOn: val ? true : false,
                    userId: { resourceId: this.device.user_id }
                }
            }
        };

        this.device.privacy_mode_on = val ? true : false;

        this.conn.protobufSendCommand([ cmd ], 'DEVICE_' + this.device.device_id);
    }
